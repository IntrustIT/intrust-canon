...
- Signature switch — **OPTIONAL slot, ≤1 per page**. Use ONLY when the page has an obvious page-distinctive binary filter that's flipped multiple times per session AND reshapes the user's mental view of the list. If no obvious candidate, **skip the slot entirely** — don't search for one to fill. When used, inline only — never duplicates inside Filters popover. /todos has no signature switch; /issues had Stractical Only as a candidate which is now retired in favor of persistent-section grouping (`reference_persistent_section_grouping.md`), leaving /issues with no signature switch either. Most pages will not need this slot.
